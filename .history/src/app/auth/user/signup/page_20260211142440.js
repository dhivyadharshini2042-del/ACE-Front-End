        ...
        <div className="auth-card signup-card">
          ...
          <form onSubmit={onSubmit}>
            ...
            <input
              ...
              value={form.name}
              onChange={(e) => {
                const value = e.target.value.slice(0, 250); // restrict to 250 chars
                setForm({ ...form, name: value });
              }}
            />
            {/* Warning / Character count */}
            {/* <small
              className={`text-${form.name.length >= 250 ? "danger" : "muted"}`}
              style={{ display: "block", marginTop: "4px" }} // ✅ forces below input
            >
              {form.name.length}/250 {form.name.length >= 250 && "- Maximum reached!"}
            </small> */}
            ...
            {/* Show message ONLY when limit reached */}
            {form.name.length === 250 && (
              <small
